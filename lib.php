...
/**
 * TinyMCE text editor import Microsoft Word files.
 *
 ...
 * @return void
 */
function tinymce_wordimport_strings_for_js() {
    global $PAGE;
    ...
    $PAGE->requires->strings_for_js($strings, 'tinymce_wordimport');
}
...
function tinymce_wordimport_convert_to_xhtml($filename, $contextid, $draftitemid) {
    ...
    if ($zfh) {
        ...
        tinymce_wordimport_debug_unlink($filename);
        return false;
    }
    ...
    if (!($xsltoutput = xslt_process($xsltproc, $tempwordmlfilename, $word2mqxmlstylesheet1, null, null, $parameters))) {
        ...
        tinymce_wordimport_debug_unlink($tempwordmlfilename);
        return false;
    }
    tinymce_wordimport_debug_unlink($tempwordmlfilename);
    ...
    if (!($xsltoutput = xslt_process($xsltproc, $tempxhtmlfilename, $word2mqxmlstylesheet2, null, null, $parameters))) {
        ...
        tinymce_wordimport_debug_unlink($tempxhtmlfilename);
        return false;
    }
    tinymce_wordimport_debug_unlink($tempxhtmlfilename);
 ...
 */
function tinymce_wordimport_get_html_body($xhtmlstring) {
 ...
 */
function tinymce_wordimport_debug_unlink($filename) {
